      ...
      * ------------------------------------------------------------ *
      *  JSN 03/12/2019 - Se agrega el tag Solicitud Web.            *
      *                                                              *
      * ************************************************************ *
     Fpahsra01  if   e           k disk
     Fpahed3    if   e           k disk
     Fpahed0    if   e           k disk
     Fpahet0    if   e           k disk
     Fset001    if   e           k disk
     ...
     D  this                         10a   overlay(PsDs:1)
       ...
       *inlr = *on;
       ...
       desd = REST_getNextPart(url);
       hast = REST_getNextPart(url);

       monitor;
         peFema = %dec( %subst(desd:1:4) : 4 : 0);
        on-error;
         peFema = *year;
       endmon;

       monitor;
         peFemm = %dec( %subst(desd:6:2) : 2 : 0);
        on-error;
         peFemm = *month;
       endmon;

       monitor;
         peFemd = %dec( %subst(desd:9:2) : 2 : 0);
        on-error;
         peFemd = *day;
       endmon;

       monitor;
         p2Fema = %dec( %subst(hast:1:4) : 4 : 0);
        on-error;
         p2Fema = *year;
       endmon;

       monitor;
         p2Femm = %dec( %subst(hast:6:2) : 2 : 0);
        on-error;
         p2Femm = *month;
       endmon;

       monitor;
         p2Femd = %dec( %subst(hast:9:2) : 2 : 0);
        on-error;
         p2Femd = *day;
       endmon;

       peFemi = (peFema * 10000) + (peFemm * 100) + peFemd;
       p2Femi = (p2Fema * 10000) + (p2Femm * 100) + p2Femd;
       ...
       k1hsra.rafema = peFema;
       k1hsra.rafemm = peFemm;
       k1hsra.rafemd = peFemd;

       COWLOG_logcon('WSRMEP':peBase);
       ...
       setll %kds(k1hsra:7) pahsra01;
       reade %kds(k1hsra:4) pahsra01;
       dow not %eof;
